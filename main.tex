...
With this identification, the corestriction map is induced by 
 the identity map $\opn{Id}$ on $\calD(V)$. Since $\frac{\calD(V)}{\psi-1}$ is compact (cf. [TBA]), one concludes that
 $$H^2_{\opn{Iw}}(K,V)=\varprojlim_{n}\frac{\calD(V)}{\psi-1}/(\gamma_n-1)=\frac{\calD(V)}{\psi-1},$$
 where the last equality is [TBA].

 The case of $i=1$ is of most interests. It is easy to check that the exact sequence \Cref{eq:57298} admits following naturlity:
 $$\begin{tikzcd}
 ...
 \end{tikzcd}.$$
 Thus by taking inverse limit on \Cref{eq:57298}, we get
 $$\begin{tikzcd}
     0\ar[r]&\varprojlim_n\frac{\calD(V)^{\psi-1}}{\gamma_n-1}\ar[r]&H^1_{\opn{Iw}}(\scrG_{K_n^{\opn{cycl}}},V)\ar[r]& &0
 \end{tikzcd}$$
    \end{enumerate}
\end{proof}
