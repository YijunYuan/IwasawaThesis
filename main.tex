...
We have the following properties of these period rings of characteristic $0$. Most of them follows from the universal properties of the Witt vectors and the Cohen ring:
\begin{proposition}\leavevmode
	\begin{enumerate}
...
Since we have established the equivalence of categories \Cref{thm:24308}, it is natural to use the \'etale $(\varphi,\Gamma)$-modules to recover the informations of the $p$-adic Galois representations. For example, we can use \'etale $(\varphi,\Gamma)$-modules to calcuation the Galois cohomology, and this is discovered by Herr:
\begin{theorem}[{\cite{herr_sur_1998}}]
...
\end{lemma}
\begin{proof}
    Define $\psi$ on $D$ to be the operator $\psi\otimes 1$ on $\bfA\otimes_{A_K}\calV(D)$.  The uniqueness follows from the \'etaleness of $D$.
\end{proof}
...
\end{proposition}
\begin{proof}
    We only need to consider the case that $V$ is a $\bbZ_p$-representation of $\scrG_K$.
\end{proof}
